...
def bulk_pred_2step(folder_path, th=0.5, steps=False, out_dir='out'):
    ...
    if len(imgs) != len(os.listdir(folder_path)):
        logger.warning('some files where excluded from input dir because where not recognized as images extensions.')

    result = {}
    for img in imgs:
        try:
            img_class = img_pred_2step(img, th, steps, out_dir)
            result.update(img_class)
        except Exception as e:
            logger.error('failer to process image', img_name)
    return result
...
def img_pred_2step(img_path, th=0.5, steps=False, out_dir='out'):
    ...
    if steps:
        ...
        cv2.imwrite(join_path(out_dir,'wind_' + img_name + ext), ann_img)

    # combine output
    avg = np.mean(out_class, axis=0)
    final_pred = final_pred_map[np.argmax(avg)]
            
    return {img_name+ext:final_pred}
